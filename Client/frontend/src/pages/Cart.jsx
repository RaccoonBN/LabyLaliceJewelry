...
const Cart = () => {
    ...
    useEffect(() => {
        ...
        axios
            ...
            .then((response) => {
                // Thêm product_id vào mỗi item (nếu chưa có)
                const cartData = response.data.map((item) => ({
                    ...item,
                    product_id: item.product_id || item.id, // Lấy product_id hoặc dùng id nếu không có
    ...
    const handleCheckout = () => {
        const selectedItemsWithProductId = cartItems
            .filter((item) => item.checked)
            .map(item => ({
                ...item,
                product_id: item.product_id || item.id // Đảm bảo có product_id
            }));
        
        // Bây giờ selectedItemsWithProductId đã được định nghĩa, bạn có thể sử dụng nó
        console.log("checkoutItems trước khi lưu vào localStorage:", selectedItemsWithProductId);
        localStorage.setItem("checkoutItems", JSON.stringify(selectedItemsWithProductId));

        if (selectedItemsWithProductId.length === 0) {
